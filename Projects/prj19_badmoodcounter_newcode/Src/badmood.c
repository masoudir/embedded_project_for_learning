...
//  show the separat line
   // char BUFFER_line[100]={0};
    sprintf(BUFFER_line, "-----------------------------------------------------------------\r\n");
    HAL_UART_Transmit(huart, (const uint8_t*)BUFFER_line, 100);
   }
    ...
    if(input->lcdmode==LCD_mode_1) {
        ...
        HAL_UART_Transmit(huart, (const uint8_t*)BUFFER_homescreen, 100);
    ...
    // show current badmoodcount
        ...
        HAL_UART_Transmit(huart, (const uint8_t*)BUFFER_badmoodwords, 100);
        HAL_UART_Transmit(huart, (uint8_t*)input->count, 100);

     // show late seven days records
        sprintf(BUFFER_7days, "Below is last 7days record\r\n");
        HAL_UART_Transmit(huart, (const uint8_t*)BUFFER_7days, 100);

       for(int i=0; i<7;i++) {    
          HAL_UART_Transmit(huart, (uint8_t*)input->array, 100);
        }
   ...
   if(input->lcdmode==LCD_mode_2){
        ...
        HAL_UART_Transmit(huart, (const uint8_t*)BUFFER_settingscreen, 100);
    } 
}