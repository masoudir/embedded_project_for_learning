 ...
 */
/**************************************************************************/
VOID MFRC522(int sad, int reset) {
  _sad = sad;
...
HAL_StatusTypeDef RFID_SPI_TransmitReceive(ACC_SPI_TX_Frame_t* pTxData, ACC_SPI_RX_Frame_t* pRxData, uint32_t Timeout) {
    HAL_StatusTypeDef status;
    HAL_GPIO_WritePin(RFID_spi_config.CS_PORT, RFID_spi_config.CS_PIN, GPIO_PIN_RESET);
    status = HAL_SPI_TransmitReceive(&hspi2, (uint8_t*)pTxData, (uint8_t*)pRxData, 2, Timeout);
    HAL_GPIO_WritePin(FRID_spi_config.CS_PORT, RFID_spi_config.CS_PIN, GPIO_PIN_SET);
    return status;
