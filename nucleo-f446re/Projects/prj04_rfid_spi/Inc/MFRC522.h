...
#define MFRC522_SPI_TIMEOUT_MS 100

typedef struct
{
    GPIO_TypeDef *port; // PORT NAME OF CS
    uint16_t pin;       // PIN NAME OF CS
} MFRC522_GPIO_Details_t;

typedef struct
{
    MFRC522_GPIO_Details_t cs;
    MFRC522_GPIO_Details_t mosi;
    MFRC522_GPIO_Details_t miso;
    MFRC522_GPIO_Details_t clk;
} MFRC522_SPI_Config_t;
...
int MFRC522_commandTag(byte command, byte *data, int dlen, byte *result, int *rlen);
int MFRC522_requestTag(byte mode, byte *type);
...
int MFRC522_readFromTag(byte blockAddr, byte *recvData);
int MFRC522_writeToTag(byte blockAddr, byte *writeData);
int MFRC522_haltTag();
